<div class="modal-event">
  <form action="{{route('send.modal')}}" method="post" class="modal-event-wrap">
    @csrf
    <div class="btn-close">
      <svg
        width="28"
    ...
    <div class="inputs">
      ...
      <div class="middleBlock">
        <div class="input-group">
          <label>Дата</label>
          <input type="date" placeholder="Ваше имя" name="dateprovmod"/>
        </div>
        <div class="input-group">
          <label>Кол-во гостей</label>
          <input id="guest_count" type="text" name="countguestmod"/>
        </div>
      </div>
      <div class="rightBlock">
        <div class="input-group">
          <label>Комментарий</label>
          <textarea name="messmod"></textarea>
        </div>
      </div>
    </div>
    <div class="bottom">
      <input type="hidden" name="hiddenparmod" value="портфолио '{{$page['h1']}}'"/>

      <button class="order-link">Заказать</button>
      <div class="check-group">
        <input required type="checkbox" id="modal-event" />
